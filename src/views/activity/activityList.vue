<template>
  <div>
    ...
    <!--添加公告-->
    <el-dialog class="app-edit" title="添加活动" :visible.sync="dialogFormVisible" :before-close="handleClose">
    ...
    <div class="app-container calendar-list-container">
      <div class="filter-container" v-show="this.roles[0]==='admin'">
        <el-button class="filter-item" style="margin-left: 10px;" @click="handleClick" type="primary" icon="plus">添加活动
        </el-button>
      </div>
      <br/>
      <!--表格-->
      <el-table :data="tableData" v-loading.body="listLoading" element-loading-text="给我一点时间" stripe border fit
        ...
        <el-table-column align="center" prop="url" label="图片"></el-table-column>
        <el-table-column align="center" prop="created_date" label="创建时间"></el-table-column>
        <el-table-column align="center" label="操作" max-width="320" v-if="this.roles[0]==='admin'">
        ...
        <el-table-column align="center" label="操作" max-width="320" v-else>--</el-table-column>
      </el-table>
    </div>
    <div class="block">
...
<script>
  import {activityList,addActivity,activityDetail,deleteActivity} from '@/api/agent'
  ...
  export default {
    ...
    methods: {
      ...
      getActivityList() {
        ...
        activityList(this.pageNumber,this.page_size).then(response => {
      ...
      handleEdit(id) {
        ...
        this.editFlg = true;
        activityDetail(id).then(response => {
      ...
      doAdd(formName){
        ...
        this.$refs[formName].validate((valid) => {
          //本地验证
          if (valid) {
            addActivity(params, 'POST').then(response => {
              this.$message({
                message: '操作成功',
                type: 'success'
              });
